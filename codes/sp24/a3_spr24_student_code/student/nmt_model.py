...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings(source_padded, True)  # returns (src_len, b, e) since embedding is input (*) -> output (*, H = embedding_dim)
        ...
        X = torch.permute(X, (1, 2, 0))  # Permute to (b, e, src_len) since our CNN model's input is (b, e, src_len)
        X = self.post_embed_cnn(X)  # Output (b, e, src_len)
        X = torch.permute(X, (2, 0, 1)) # Revert to (src_len, b, e)
        ...
        packed_input = pack_padded_sequence(X, source_lengths, enforce_sorted=False, batch_first=False) # We need to tell LSTM to ignore the output cell and hidden states of the <pad> token in the sequence

        packed_enc_hidden_states, (hn, cn) = self.encoder(packed_input) # hn , cn are the final hidden state
        enc_hiddens, _ = pad_packed_sequence(packed_enc_hidden_states, batch_first=False) # (src_len, b, h*2) since batch_first=False
        ...
        init_decoder_hidden = self.h_projection(torch.cat((hn[0], hn[1]), dim=-1)) # shape(b, 2*h)
        init_decoder_cell = self.c_projection(torch.cat((cn[0], cn[1]), dim=-1)) # shape(b, 2*h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings(target_padded, False)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, dim=-1)), dim=-1) # output (b, src_len)
        ...
        # Set e_t to -inf where enc_masks has 1, making sure that the softmax will assign very small prob to the masked position
        # This effectively prevent the model from attending to the <pad> hidden states
        if enc_masks is not None:
            e_t.data.masked_fill_(enc_masks.bool(), -float('inf'))

        ### YOUR CODE HERE (~6 Lines)
        ###     1. Apply softmax to e_t to yield alpha_t
        alpha_t = torch.softmax(e_t, dim=-1) # outputs (b, src_len)
        ...
        a_t = torch.squeeze(torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens), dim=1) # output (b, 2h)

        ###     3. Concatenate dec_hidden with a_t to compute tensor U_t
        U_t = torch.cat((a_t, dec_hidden), dim=-1) # output (b, 3h)

        ###     4. Apply the combined output projection layer to U_t to compute tensor V_t
        V_t = self.combined_output_projection(U_t) # Output (b, h)
